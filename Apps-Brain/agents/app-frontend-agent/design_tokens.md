...
### 8.9 Making domain tokens theme-aware — the `@theme inline` indirection

§2 promotes severity to `--color-severity-*` tokens. Done naively that produces a **broken dark
mode**: the severity ramp stays at its light values, and a `red-600` (`#dc2626`) alarm on a
`#1e293b` card is **3.4:1** — dimmer than the body text beside it.

Do **not** try to fix it by remapping `.text-severity-critical` in the retrofit block (§8.4);
the utility is generated by Tailwind and the token is the thing that should move. Use a two-step
indirection instead `[observed 2026-09-01]`:

```css
:root, [data-theme='light'] { --sev-critical: #dc2626; /* … */ }
[data-theme='dark']         { --sev-critical: #f87171; /* 5.3:1 on the card */ }

@theme inline {
  --color-severity-critical: var(--sev-critical);
}
```

`inline` is what makes this work: the generated utility emits the **inner** variable at the use
site rather than a value resolved once at `:root`, so it re-resolves against whichever theme is
active. Without `inline`, the utility bakes in the light value and the dark override is dead.

**Evidence:** built stylesheet after `vite build` contains
`.text-severity-critical{color:var(--sev-critical)}` alongside both
`:root,[data-theme=light]{--sev-critical:#dc2626;…}` and
`[data-theme=dark]{--sev-critical:#f87171;…}` — verified by grepping `dist/assets/main-*.css`
in `Fab-Network-Operations/app` on 2026-09-01. Note the minifier **strips the quotes** from the
attribute selector (`[data-theme=dark]`), so grep for the unquoted form when checking a build.

Guard it with a test — a broken dark ramp is invisible to anyone developing in light mode:

```ts
expect(darkVars).toContain('--sev-critical:');
