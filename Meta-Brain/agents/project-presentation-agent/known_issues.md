...
### Problem: the pre-commit `/markdown` check fails with "text is too large" on a 19 KB README

- **Root Cause**: On Windows the README is CRLF, and `gh api -X POST /markdown` rejects a CRLF payload with `HTTP 403 - This API renders Markdown text up to 400 KB in size. The requested text is too large to render`. The message is simply wrong: the same document, same byte count, renders fine once the CRs are stripped. What the message costs is the **verification step itself** - the previous entry prescribes running `/markdown` before committing, and this failure looks like a hard platform limit, so the natural reaction is to shrug and commit unverified. That is the real damage: a misleading error turns a mandatory check into an optional one. Note the size in the message is a red herring twice over - the 400 KB limit is real, but it is not what fired here. Why CRLF trips it was **not established**; only that it does, reproducibly.
- **Solution**: Normalise before sending, never trust the message: `(Get-Content README.md -Raw) -replace "`r`n","`n"`. Write the JSON body with `[System.IO.File]::WriteAllText(path, json, (New-Object System.Text.UTF8Encoding $false))` - `Out-File -Encoding utf8` on PowerShell 5.1 emits a BOM, which is a separate way to corrupt the same call. If a "limit" error arrives on a payload nowhere near the limit, **measure the payload** before believing it; here `md.json` was 21.4 KB against a 400 KB ceiling, which falsifies the message in one command and points the search elsewhere.
- **Evidence**: `Azure-Brain` README, 2026-09-02. Controlled A/B on one variable: identical 354-line text, CRLF payload -> exit 1 with the 403; the same text with every CR stripped (CRLF -> LF) -> exit 0 and 418 lines of HTML containing `<video src="https://private-user-images.githubusercontent.com/...">`. Bisection by line count (100/200/300/354) passed at every size **because the chunking re-joined the lines on LF**, which was itself the accidental clue that size was not the variable. `README.md` measured 229 CRLF pairs, `md.json` 21.4 KB against the 400 KB ceiling the error message named.

### Problem: the public-safety scanner flags a GitHub attachment id as a tenant GUID

- **Root Cause**: The attachment id in a `user-attachments/assets/<guid>` URL is a well-formed GUID, and a secret scanner cannot tell it from a tenant, subscription or workspace id. So the moment the teaser is embedded, `test_no_warning_findings` goes red on a value that is public by construction - the README that carries it is public, and the id is worthless without GitHub's signed redirect. The trap is the obvious fix: redacting the GUID or relaxing the rule. Both are wrong. Redacting it breaks the player; relaxing the GUID rule blinds the scanner to the real tenant ids it exists to catch.
- **Solution**: Allowlist the single value in `.publicsafety-allow` **with its reason**, the way the repo already does for public Azure role and appId constants. An allowlist entry that explains why a value is safe stays auditable; a weakened regex does not. Expect this failure as a normal step of embedding a video - it is the guard working, not a defect.
- **Evidence**: `Azure-Brain`, 2026-09-02. `tests/test_public_safety.py::test_no_warning_findings` failed with `README.md:43 [real-guid] 4bdba31c-...` on the commit that added the teaser; after appending the id and a six-line justification to `.publicsafety-allow`, the three README-touching suites returned `858 passed`. Published README verified afterwards with `gh api /repos/Statyx/Azure-Brain/readme -H "Accept: application/vnd.github.html+json"` - `<video>` present.